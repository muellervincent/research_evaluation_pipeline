...
class ResultBuilder:
    ...
    def build_final_result(
        ...
        for assessment_answer in assessment_report.answers:
            original_identifier = identifier_mapping.get(assessment_answer.question_id) or assessment_answer.question_id
        ...
        accuracy = (correct_prediction_count / total_question_count) if total_question_count > 0 else 0.0

        metrics = PipelineMetrics(
            total_questions=total_question_count, correct_predictions=correct_prediction_count, accuracy=accuracy
        )
    ...
    def build_markdown_report(result: FinalPipelineResult, profile: PipelineProfile) -> str:
        ...
        for question_result in result.results:
            ground_truth_display = str(question_result.ground_truth_answer) if question_result.ground_truth_answer is not None else "N/A"
            model_answer_display = str(question_result.model_answer) if question_result.model_answer is not None else "N/A"
            is_correct_display = "✅" if question_result.is_correct is True else ("❌" if question_result.is_correct is False else "➖")
        ...
        for question_result in result.results:
            report_lines.append(f"### {question_result.semantic_id} ({question_result.original_id})")
